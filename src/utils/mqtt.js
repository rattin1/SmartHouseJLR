...
const subscribeDeviceStatus = (callback) => {
  ...
  return () => {
    deviceStatusCallbacks = deviceStatusCallbacks.filter(
      (cb) => cb !== callback
    );
  };
};
...
function setupCallbacks() {
  ...
  client.onMessageArrived = (message) => {
    ...
    } else if (message.destinationName === topicoStatusSala) {
      try {
  ...
  client.connect({
    ...
    onSuccess: () => {
      ...
      client.subscribe(topicoSensor, {
        ...
        onFailure: (err) =>
          console.error("❌ Falha ao se reinscrever no sensor:", err),
      });
      ...
      client.subscribe(topicoStatusSala, {
        ...
        onFailure: (err) =>
          console.error("❌ Falha ao se inscrever no status:", err),
      });
    },
    onFailure: (err) => {
      isConnected = false;
  ...
  client.connect({
    ...
    onSuccess: () => {
      ...
      client.subscribe(topicoSensor, {
        ...
        onFailure: (err) =>
          console.error("❌ Falha ao se inscrever no sensor:", err),
      });
...
function enviarComando(topico, estado) {
  ...
  if (!isClientConnected()) {
    console.warn(
      "⚠️ Cliente não conectado. Comando será perdido:",
      topico,
      estado
    );
    addToMessageLog(
      "SISTEMA",
      `Comando perdido - desconectado: ${topico} -> ${estado}`,
      "error"
    );
    return;
  }
...
const getConnectionStatus = () => ({
  isConnected,
  hasData:
    currentSensorData.temperatura !== 0 || currentSensorData.umidade !== 0,
});
